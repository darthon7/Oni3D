...
// --- Iconos SVG ---
const IconTag = () => ( // Icono para Marca
...
const IconPaintBrush = () => ( // Icono para Color
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
...
const IconNotes = () => ( // Icono para Descripción
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
...
export default function Agregar_Pintura() {
    const [pintura, setPintura] = useState({
        marca: "",
        color: "#ffffff", // Iniciar con un valor hexadecimal por defecto
        descripcion: "",
    });
    // Nuevo estado para controlar si se usa el selector de color
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            alert("Error al guardar la pintura. Revise la consola."); // Mensaje de error
        }
    };
            ...
            >
                ...
                <form onSubmit={onSubmit} className="space-y-6">
                    ...
                    <div className="relative">
                         ...
                         {/* Contenedor para input de texto y selector */}
                         <div className="relative flex items-center gap-3">
                            <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-indigo-300 pointer-events-none">
                                <IconPaintBrush />
                            </span>
                            {/* Input de Texto (muestra el valor hexadecimal) */}
                            <input
                                type="text"
                                id="color"
                                name="color"
                                value={color}
                                onChange={onInputChange} // Permite escribir directamente
                                required
                                placeholder="Ej: #FF0000"
                                className={`w-full pl-10 pr-4 py-3 border border-indigo-400/50 bg-indigo-900/50 text-white rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 outline-none transition-all ${useColorPicker ? 'pr-12' : ''}`} // Más padding si el picker está visible
                            />
                            {/* Selector de Color Visual (condicional) */}
                            {useColorPicker && (
                                <input 
                                    type="color"
                                    value={color} // Vinculado al mismo estado 'color'
                                    onChange={onColorPickerChange} // Handler específico
                                    ...
                                    style={{ padding: 0 }} // Algunos navegadores necesitan esto
                    ...
                    <div className="relative">
                        ...
                        <div className="relative">
                            <span className="absolute top-3 left-0 flex items-center pl-3 text-indigo-300 pointer-events-none"> {/* Ajuste de 'inset-y-0' a 'top-3' */}
                                <IconNotes />
                            </span>
                            <textarea
                                id="descripcion"
